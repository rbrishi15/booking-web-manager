...
describe("Money", () => {
  ...
  test("adds and subtracts signed values without changing either operand", () => {
    ...
    const combined = difference.add(first);
    const subtractingNegative = first.subtract(Money.fromCents(-100));
    const addedZero = first.add(Money.fromCents(0));
    const subtractedZero = first.subtract(Money.fromCents(0));

    // Assert
    expect(sum.toCents()).toBe(1200);
    expect(difference.toCents()).toBe(-200);
    expect(combined.toCents()).toBe(300);
    expect(subtractingNegative.toCents()).toBe(600);
    expect(first.toCents()).toBe(500);
    expect(second.toCents()).toBe(700);
    expect(addedZero).not.toBe(first);
    expect(subtractedZero).not.toBe(first);
